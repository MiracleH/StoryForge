...
- 每个场景生成尽量多的镜头（建议 8-16 个），系统会自动按 4 个一组合并
        ...
        if (sceneSb.shots && Array.isArray(sceneSb.shots)) {
          ...
          for (let i = 0; i < sceneSb.shots.length; i += GROUP_SIZE) {
            ...
            const title = group.length === 1
              ? (group[0].title || `镜头组 #${totalGenerated + 1}`)
              : `${group[0].title || '镜头' + (i + 1)} ~ ${group[group.length - 1].title || '镜头' + (i + group.length)}`;
            ...
            // 取第一个镜头的景别和运镜
            const cameraAngle = group[0].camera_angle || '中景';
            const cameraMovement = group[0].camera_movement || '固定';
